...
export default function Login() {
  ...
  const [schoolCode, setSchoolCode] = useState(""); // ✅ added
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPw, setShowPw] = useState(false);
  const [loading, setLoading] = useState(false);

  // optional: remember last email to reduce friction
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // ✅ pass cleaned values + optional schoolId
      const data = await login({
        email: cleanEmail,
        password: cleanPassword,
        schoolId: cleanSchoolCode || null,
      });
      ...
      // optional but useful (instant role-based UI while /me loads)
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Leave blank only if you are SYSTEM_ADMIN.
                  </div>
                </div>
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  school’s data. Unauthorized access is prohibited.
                </div>
              </form>
            </CardContent>
          </Card>
